...
// > cd \
// > cd xampp\mysql\bin
// > mysql --host=mcldisu5ppkm29wf.cbetxkdyhwsb.us-east-1.rds.amazonaws.com --user=me3vsztjpki873rv --password=kbx3ndxm1m3xs0ru --reconnect  mk5ezo38qe9k5wyu < C:\xampp\htdocs\_tmp\chat\_\chat.sql
...
function getLogs(socket, data, fromId, toId) {
    //var sql = 'DELETE FROM `'+ prefix +'chats` WHERE DATE( `created_on` ) < DATE_SUB( CURDATE(), INTERVAL 1 DAY ); ';
    var sql = 'SELECT * FROM `'+ prefix +'chats` WHERE ( `msg_from` = "'+ fromId +'" AND `msg_from` = "'+ toId +'" ) OR ( `msg_to` = "'+ fromId +'" AND `msg_to` = "'+ toId +'" ) AND status = "1" ORDER BY `created_on` ASC LIMIT 10; ';
        ...
        switch( data.act ) {
            ...
            case 'logs' :
                ...
                if( typeof groupId == 'undefined' ) {
                    ...
                    var   sql = 'SELECT `id` FROM `'+prefix+'groups` WHERE `group_type` != "1" AND ( `group_members_id` = "'+ fromId +'" OR `group_members_id` = "'+ toId +'" OR `created_by` = "'+ fromId +'" OR `created_by` = "'+ toId +'" ) AND status = "1"';
                    con.query(sql, function(err, rows, fields){
                        ...
                        if( rows != '' ) {
                            ...
                            * Get all prev. chats
                            */
                            getLogs(socket, data, fromId, toId);


                        }else {
                            // Insert New Pipe
                            var sql = 'INSERT INTO `'+prefix+'groups` (`group_type`, `group_name`, `group_slug`, `group_members_id`, `created_by`, `created_on`) VALUES("3", "Pipe", "pipe-'+ fromId +'", "'+ toId +'", "'+ fromId +'", "'+ now() +'");';
                            con.query(sql, function(err, rows, fields){ 
                                var sql = 'SELECT MAX(`id`) AS id FROM `'+prefix+'groups`; ';
                                con.query(sql, function(err, rows, fields){ 
                                    groupId = rows[0]['id'];
 ...
 * Set
 */
function _set(socket){
    socket.on('set', function(args){
        ...
        switch( args.type ) {
            case 'options' :
                 ...
                 * Set User Options
                 */
                var sql = 'SELECT `options` FROM `'+ prefix +'users` WHERE `id` = "'+ args.user.id +'"';
                con.query(sql, function(err, rows, fields){
                    if( err ) throw err;
                    
                    if( rows != '' ) {
                        var opts = JSON.parse(rows[0]['options']) || {};
                        Object.keys(opts).map(function(i, v){
                            if( args.data[i] ) {
                                opts[i] = args.data[i];
                            }
                        })
                        
                        var sql = 'UPDATE `'+ prefix +'users` SET `options` = "'+ JSON.stringify( opts ).toString().replace(/"/g, '\\\"') +'" WHERE `id` = "'+ args.user.id +'"';
                        con.query(sql, function(err, rows, fields){ });
                    }
...
function _join(socket){
    ...
    socket.broadcast.to(group).emit('log', {
        status  : true,
        msg     : 'Some one has joined on ' + group
    });
                                ...
                                if( rows == '' ) {
                                    ...
                                    con.query(sql, function(err, rows, fields){
                                        ...
                                        var args = {
                                            ...
                                            options     : '{\"themeClr\":\"rgba(15, 157, 88, 1)\"}',
                                            created_on  : now(),
                                        };
...
function _query(tblName, args, cond) {
    ...
    if( typeof cond == 'undefined' ) {
        ...
        Object.keys(args).map(function(clm, val){
            clms += '`'+ clm +'`, ';
            vals += '"'+ args[clm].toString().replace(/"/g, "\\\"") +'", ';
        })
